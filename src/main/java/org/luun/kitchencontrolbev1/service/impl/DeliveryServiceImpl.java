...
import org.luun.kitchencontrolbev1.service.statustransitionhandler.DeliveryStatusTransitionHandler;
import org.luun.kitchencontrolbev1.service.statusvalidator.DeliveryStatusValidator;
...
public class DeliveryServiceImpl implements DeliveryService {
    ...
    private final DeliveryStatusTransitionHandler deliveryStatusTransitionHandler;
    ...
    @Override
    @Transactional
    public void createDelivery(AssignShipperRequest request) {
        if(request.getOrderIds() == null || request.getOrderIds().isEmpty()){
            throw new IllegalStateException("There is no order to assign");
        }

        User shipper = userService.getUserById(request.getShipperId());
        ...
        orders.forEach(savedDelivery::addOrder);
    }

    @Override
    @Transactional
    public void updateDeliveryStatus(Integer deliveryId, DeliveryStatus status) {
        Delivery delivery = getDeliveryById(deliveryId);

        deliveryStatusValidator.validate(delivery.getStatus(), status);

        deliveryStatusTransitionHandler.handle(delivery, status);
